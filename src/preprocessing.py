...
def process():
    ...
    df = df.rename(columns=names_mapper_dict)
    ...
    variables_for_lag_list = ['EC ug/m3', 'OC ug/m3', 'PM10 ug/m3', 'NO2 ug/m3', 'Weekday','Temperature max',
                              'Temperature min', 'Temperature average', 'Temperature max-min', 'Pressure max',
                              'Pressure average', 'Relative humidity max', 'Relative humidity min',
                              'Relative humidity average', 'Relative humidity max-min', 'Wind speed', 'Precipitation',
                              'Wind direction']

    #create lag values
    X_lagged = X0[variables_for_lag_list].shift(+1)
    X_lagged.columns = [str(col) + '_lag' for col in X_lagged.columns]
    #concat lag to X
    X = pd.concat([X0, X_lagged], sort=True, axis=1)
    X.dropna(how='all', inplace=True)

    return X

#dictionary for mapping long and short names
names_mapper_dict = {'meteo_temp_max': 'Temperature max', 'meteo_temp_min': 'Temperature min', 'meteo_temp_mean': 'Temperature average',
